![:name](https://count.getloli.com/@astrbot_plugin_x_score?name=astrbot_plugin_x_score&theme=green&padding=7&offset=0&align=top&scale=1&pixelated=1&darkmode=auto&prefix=0)

# X账号评分 - AstrBot 插件

查询 X(Twitter) 账号可信度评分，基于 [flj.info](https://flj.info) 的 AI 分析。

![Preview](https://github.com/xiaoruange39/Plugin-Preview-Image/blob/main/image/Image_1777519815672_272.png)

## 功能

- 输入 X 用户名，自动调用 flj.info API 进行可信度分析
- 生成包含评分、标签、评分明细、AI 评价、近期媒体、用户评论的图片报告
- 支持自动撤回（可配置延迟）
- 图片生成失败时自动回退为纯文本

## 使用方法

```
/X账号评分 <用户名>
```

示例：

```
/X账号评分 elonmusk
```

### 随机查询

从 [flj.info](https://flj.info/rankings) 排行榜中随机抽取一个账号进行评分查询，无需输入用户名：

```
/X随机查询
```

## 配置项

| 配置项 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `recall_delay` | int | 0 | 发送评分图片后多少秒自动撤回，0 表示不撤回 |
| `show_analyze_alert` | bool | true | 是否在分析时发送"正在分析..."提示 |
| `font_path` | string | "" | 生成评分图片时使用的自定义字体路径，支持绝对路径或相对插件目录 / `fonts` 目录 |

## 字体

插件会优先使用配置项 `font_path` 指定的 `.ttf` / `.ttc` / `.otf` 字体文件。可以填写绝对路径，也可以填写相对于插件目录的路径或文件名；例如把字体放到插件目录下的 `fonts` 文件夹后，配置为 `fonts/MyFont.ttf` 或 `MyFont.ttf` 均可。

留空时插件会按原逻辑自动查找插件目录和系统中文字体。

## 依赖

- `aiohttp` - 网络请求
- `Pillow` - 图片生成

## 文件说明

```
├── main.py             # 插件入口，指令处理与消息发送
├── image_render.py     # 图片渲染模块，生成可信度报告图片
├── metadata.yaml       # 插件元数据
├── _conf_schema.json   # 配置项定义
└── requirements.txt    # Python 依赖
```

---

- **作者**: [xiaoruange39](https://github.com/xiaoruange39)
- **QQ群**: [QQ群](https://qm.qq.com/q/8kdJ2Bzf6S)
